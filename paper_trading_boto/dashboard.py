...
.. warning:: **NON-FUNCTIONAL — needs porting.**  This dashboard was
   written against the pre-redesign architecture and imports modules
   that no longer exist (``ibkr_interface``, ``risk_management``,
   ``cost_basis``, the old ``strategy`` module).  It is kept as a
   reference for a future port to the event-driven engine
   (``Engine``/``IBKRBroker``/``RiskManager``).  Do not run it.
